...
are combined with liquid water to form seawater in the `sea3\*` modules. The
contribution of salt to the Gibbs free energy is formulated as an expansion in
terms of salinity:

    g(S,T,p) = g1(T,p) S ln(S) + sum\_{i=2}^{NSALTERMS} gi(T,p) S^(i/2)
...
* `sal1` provides a function that calculates each of the coefficients gi in
    the salinity expansion.
* `sal2` provides thermodynamic properties of seawater that depend only on the
    salinity, such as dilution and osmotic coefficients.
