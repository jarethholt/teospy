...
__all__ = ['eq_wpte','iceair_h','cp','density','kappa_s','lapserate',
    'temperature','eq_pot','potdensity','potenthalpy','pottemp']
    ...
    if pvmax >= _PTPE:
        ...
        if entr >= ssat:
            ...
            csat = (wair*_CDRY + (1-wair)*_CVAP
                + (1-wair)*(_EPSW*wair+1-wair)/(_EPSW*wair) * _RWAT
                    * ((_AVI+_BVI)*_TTP/tsat - _BVI)**2)
            temp = tsat * (1 - (ssat-entr)/csat)
...
def iceair_h(drvw,drve,drvp,wair,pres,entr=None,temp=None,airf=None,
    ...
    if airf <= wair:
        if entr is None:
            entr = air2.entropy(wair,temp,dhum)
        h = air3c.air_h(drvw,drve,drvp,wair,entr,pres,temp=temp,dhum=dhum)
        return h
...
def _approx_pot(wair,temp,pres,ppot,airf,dhum):
    ...
    """
    pv1 = _PTPE*numpy.exp((_AVI+_BVI)*(1 - _TTP/temp)
        - _BVI*numpy.log(temp/_TTP))
    a1 = max((pres-pv1) / (pres-pv1 + _EPSW*pv1), wair)
    csat1 = (wair*_CDRY + wair*(1-a1)/a1*_CVAP + (1-wair/a1)*_CICE
        + wair*(1-a1)*(_EPSW*a1+1-a1)/(_EPSW*a1**2)*_RWAT
            * ((_AVI+_BVI)*_TTP/temp - _BVI)**2)
    rsat1 = wair*_RDRY*(1
        + (1-a1)/(_EPSW*a1)**2*(_EPSW*a1+1-a1)
            * (_EPSW*a1/(_EPSW*a1+1-a1) + (_AVI+_BVI)*_TTP/temp - _BVI))
    tpot = min(temp * (ppot/pres)**(rsat1/csat1), _TTP)
    pv2 = _PTPE*numpy.exp((_AVI+_BVI)*(1 - _TTP/tpot)
        - _BVI*numpy.log(tpot/_TTP))
    apot = max((ppot-pv2) / (ppot-pv2 + _EPSW*pv2), wair)
...
def eq_pot(wair,temp,pres,ppot,airf=None,dhum=None,apot=None,tpot=None,
    ...
    lhs2, rhs2, __, __ = iceair4a._diff_pot(apot,tpot,dhpot,wair,temp,pres,ppot,
        airf,dhum)
