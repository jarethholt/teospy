...
is uses the variable `wair`. The dry air mass fraction in humid air uses
the variable `airf`.
...
def iceair_g(drvw,drvt,drvp,wair,temp,pres,airf=None,dhum=None,
    ...
    -3.91178603885e-6
    """
    airf, __, __, dhum = iceair4a.eq_atpe(temp=temp,pres=pres,airf=airf,
        dhum=dhum,chkvals=chkvals,chktol=chktol,airf0=airf0,dhum0=dhum0,
        chkbnd=chkbnd,mathargs=mathargs)
    if airf <= wair:
        warnmsg = 'Air with the given parameters is unsaturated'
        warnings.warn(warnmsg,RuntimeWarning)
        g = air_3a.air_g(drvw,drvt,drvp,wair,temp,pres,dhum=dhum)
        return g
    w = wair / airf
