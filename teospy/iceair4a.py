...
* :func:`condensationpressure`: Pressure at which ice will condense out
  of humid air.
* :func:`frostpoint`: Temperature at which ice will condense out of
  humid air.
...
__all__ = ['eq_atpe',
    'temperature','pressure','densityair','densityvap','densityice',
    'enthalpysubl',
    'condensationpressure','frostpoint','massfractionair','sublimationpressure',
    'eq_icl','icl','ict',
...
def massfractionair(temp,pres,airf=None,dhum=None,chkvals=False,
    chktol=_CHKTOL,airf0=None,dhum0=None,chkbnd=False,mathargs=None):
    """Calculate icy air dry air fraction.
    
    Calculate the dry air mass fraction of icy air. For dry fractions
    below this, the air will be saturated.
...
def eq_icl(airf,temp,pres,dhum=None,ticl=None,picl=None,dhicl=None,
    ...
    lhs, rhs, __, __ = _diff_icl(ticl,picl,dhicl,airf,temp,pres,dhum)
    lhs.insert(0,lhs1)
    rhs.insert(0,rhs1)
    errs = list()
    for (l,r) in zip(lhs,rhs):
