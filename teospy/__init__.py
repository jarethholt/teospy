"""teospy: Python implementation of TEOS-10
...
* Testing utilities are kept in the ``tests`` submodule. This includes several
  modules for testing subgroups of ``teospy``; for example, :mod:`tests.testair`
  has functions for testing the ``air*`` and related modules. There is also the
  module :mod:`tests.testdoc` that implements docstring testing suitable for
